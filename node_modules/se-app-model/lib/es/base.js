...
class Base {
    ...
    findWithoutDuplicates(body, next) {
        handle(this.config, (err, db) => {
            if (err) {
                next(err);
            } else {
                this.handle = db;
                var index = this.collection;
                const startFrom = parseInt(body.from)
                const querySize = parseInt(body.size)
                const order = body.aggs.uniqueIds.terms.order
                db.search({
                    index: index,
                    body: body
                }, (err, result) => {
                    if (err) {
                        ...
                        const aggregations = result.aggregations.uniqueIds.buckets

                        let uniqueRecords = {}
                        let recordIds = []
                        let uniqueRecordCount = 0

                        aggregations.map((payload) => {
                            let productId = payload.key
                            let recordId = payload.recordId.buckets.pop().key
                            recordIds.push(productId)
                            uniqueRecordCount++

                            uniqueRecords[productId] = {
                                recordId: recordId
                            }
                        })

                        const products = {}
                        const productIds = recordIds.slice(startFrom, startFrom + querySize);

                        productIds.map((payload) => {
                            products[payload] = {
                                id: payload
                            }
                        })
                        ...
                        Object.keys(products).map((id) => {
                            let key = uniqueRecords[id].recordId
                            body.query.terms._id.push(key)
                        })

                        if (order.salesRankPercentage) {
                            body.sort.push({
                                "amazon.details.salesRankPercentage": {
                                    "order": "asc"
                                }
                            })
                        } else if (order.salesRank) {
                            body.sort.push({
                        ...
                        db.search({
                            ...
                            if (err) {
                                ...
                                let records = this.restructureData(rejig(data.hits.hits));
                                if (this.config.createIdKey) {
                                    records = createIdKey('id', records);
                                }
                                let endResult = {
                                    total: uniqueRecordCount,
                                    records
                                };
    ...
    paginationWithoutDuplicates(payload, next) {
        ...
        this.findWithoutDuplicates(newPayload, (err, result) => {
            if (err) {
                next(err);
            } else {
            ...
            if (err) {
                ...
                db.search({
                    ...
                    if (err) {
                        ...
                        var records = this.restructureData(rejig(hits))
                        next(null, records)
                    }
                })
            }
    ...
    deleteByQueryRaw(payload, next) {
        handle(this.config, (err, db) => {
            if (err) {
                next(err)
            } else {
                this.handle = db;
                var index = payload.index ? payload.index : this.collection;
                var type = payload.type ? payload.type : 'opportunity'
