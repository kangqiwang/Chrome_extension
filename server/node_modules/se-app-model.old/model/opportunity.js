...
class Opportunity extends Base {
			...
			db.search({
				...
				if (err) {
					...
					response.hits.hits.forEach(function (hit) {
						allRecords.push({ supplier: hit._source.supplier, amazon: hit._source.amazon });
					});
	...
	findFilteredAsins(payload, next) {
		...
		payload = Object.assign({ size: 100, start: 0 }, payload);
		...
		let thisQuery = payload.query || { 'amazon.brandRestrictionResults.resultCode': 'unchecked' };
		if (!payload.noFilter) {
			query.query.bool.must.push({ match: thisQuery }); // Only return items that haven't been checked for brand restrictions
		}
		//console.log('query: ', JSON.stringify(query, null, 2));
			...
			if (err) {
				...
				if (results && results.records) {
					...
					results = Object.keys(results.records).map((thisKey) => {
						...
						records.push({ 'asin': thisItem.amazon.asin, '_id': thisItem.id, '_type': thisItem.supplier.domain.replace(/\./g, '-') });
					});
					result = {
	...
	deleteRestrictedAsins(payload, next) {
		...
		let query = {
			...
			query: {
				bool: {
					must: [{
						"match": { "amazon.brandRestrictionResults.resultCode": "Error" }
					}]
				}
			}
		};
			...
			if (err) {
				...
				if (results.total === 0) {
					...
					results.records.map((thisOpportunity) => {
						oppsToDelete.push({ "delete": { _index: this.collection, _type: 'opportunity', _id: thisOpportunity.id } })
	...
	fixWishListForBrandRestrictions(payload, next) {
		var p = {
			query: {
				bool: {
					must_not: [{
						exists: { field: 'amazon.brandRestrictionResults' }
					}]
				}
			}
		};
		...
		this.find(p, (err, result) => {
			if (err) {
				...
				result.records.map((record) => {
					...
					bulk.push({ update: { _index: this.collection, _type: type, _id: record.id } });
					bulk.push({ doc: { amazon: { brandRestrictionResults: record.amazon.brandRestrictionResults } } });
				});
	...
	bulkSaveOpportunity(opportunities, domain, next) {
		this.bulkAdd({ action: 'insert', data: opportunities, domain: domain }, next);
		// try {
		// 	var idLine = {
		// 		index: {
