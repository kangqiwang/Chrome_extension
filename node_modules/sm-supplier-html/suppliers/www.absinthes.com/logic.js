...
var Product = {
    ...
    price: function($) {
        price = $('span.price-standard').html();
...
var Search = {
    ...
    name: function($, product) {
        var name = $(product).find('div.product-name h4 a').text();
        name = cleanName(name);
        return name;

    },
    price: function($, product) {
        var price = $(product).find('div.price span:last-child').text();
        price = getPrice(price);
        return price;
    },
    ...
    url: function($, product) {
         var url = null;
        var anchor = $(product).find('div.product-name h4 a');
        if(anchor) {
...
var Category = {
    name: function($) {
        var name = $('h1.all-real-absinthe ').text();
        name = cleanName(name);
        return name;
    }
}
