module.exports = (payload, version) => {
	...
	if(payload.ignoreSmId) {
		body.query.bool.must_not.push({
			"match": {
				"supplier.id": payload.ignoreSmId
			}
		});
	}
		...
		if (payload.filter[key].state === 'on') {
			switch (key) {
				...
				case 'hideChecked':
					...
					break;
				case 'isAmazonSelling':
					body.query.bool.must_not.push({
						"match": {
							"productChecks.isAmazonSelling": true
						}
					})
				break;
				case 'hideNoCat':
	...
	if(payload.filter.matchType !== 'none') {
		body.query.bool.must.push({
			"match": {
				"supplier.matchType" : payload.filter.matchType
			}
		})
	}
