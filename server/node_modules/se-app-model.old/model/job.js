...
const createJobIdName = require('../lib/createJobIdName');
...
class Job extends Base {
    ...
    completeJobsXDays(message, next) {
        let payload = {
            $and: [{
                    $or: [{status: "COMPLETED"}, {status: "COMPLETE"}]
                }, {
                    created: {
    ...
    findCurrentSearchForUser(payload, next) {
    ...
    findOlderThanXDays(payload, next) {
        ...
        this.find({
            ...
            sort: { 'created': 1 },
            $or: [{status: "COMPLETE" }, {status: "COMPLETED"}, { status: "ERROR" }, { status: "STOPPED" }]
        }, (err, result) => {
            if (err) {
                next(err)
            } else {
    ...
    saveSupplierSearch(payload, next) {
        this._saveCommonJob(saveJobSupplierPayload(payload), (err, newJob) => {
            if(err) {
                next(err)
            } else {
               next(null, newJob) ;
    ...
    getUserIdWhereJobRunning(next) {
        this.find({ status: 'IN_PROGRESS' }, (err, result) => {
            if (err) {
                next(err);
            } else {
                let userIds = [];
                result.records.map((thisResult) => {
                    if (thisResult.userId && (thisResult.userId + '').indexOf('crawler') === -1 && thisResult.userId.length === 24) {
                        userIds.push(mongodb.ObjectID(thisResult.userId));
                    }
                });
                next(null, userIds);
            }
    ...
    getUserQueuedJobs(payload, next) {
        ...
        this.find({ userId: payload.userId, country: payload.country, status: 'QUEUED', sort: { created: 1 } }, (err, result) => {
            if (err) {
                next(err)
            } else {
             //   console.log(result)
    ...
    setInProgress(payload, next) {
        let update = {
            status: "IN_PROGRESS",
            started: new Date()
        }
        this._updateStatus(payload.jobId, update, next)
    }

    setStopped(payload, next) {
        let update = {
            status: "STOPPED",
            finished: new Date()
        }
    ...
    setComplete(payload, next) {
        let update = {
            status: "COMPLETE",
            analysed: payload.total,
            finished: new Date(payload.timestamp)
        }
        this._updateStatus(payload.jobId, update, next)
    }

    _updateStatus(jobId, data, next) {
        this.update({
            query: {
                _id: mongodb.ObjectID(jobId)
            },
